...
//import org.openmrs.module.intelehealth.htmlformentry.CodedOrFreeTextObsTagHandler;
//import org.openmrs.module.intelehealth.htmlformentry.EncounterDiagnosesTagHandler;
//import org.openmrs.module.intelehealth.htmlformentry.EncounterDispositionTagHandler;
...
/**
        if (ModuleFactory.isModuleStarted("htmlformentry")) {
            ...
            EncounterDiagnosesTagHandler encounterDiagnosesTagHandler = IntelehealthActivator.setupEncounterDiagnosesTagHandler(conceptService, adtService, emrApiProperties, uiUtils);
            htmlFormEntryService.addHandler(IntelehealthConstants.HTMLFORMENTRY_ENCOUNTER_DIAGNOSES_TAG_NAME, encounterDiagnosesTagHandler);

            EncounterDispositionTagHandler encounterDispositionTagHandler = IntelehealthActivator.setupEncounterDispositionTagHandler(emrApiProperties, dispositionService, adtService, featureToggles);
            htmlFormEntryService.addHandler(IntelehealthConstants.HTMLFORMENTRY_ENCOUNTER_DISPOSITION_TAG_NAME, encounterDispositionTagHandler);
   ...
   */
	...
	public void started() {
        ...
        GlobalProperty defaultDashboard = Context.getAdministrationService().getGlobalPropertyObject(IntelehealthConstants.GP_DEFAULT_DASHBOARD);
	...
	public void willStop() {
		log.info("Stopping Intelehealth Module");
	}
	...
	public void stopped() {
        try {
            HtmlFormEntryService htmlFormEntryService = Context.getService(HtmlFormEntryService.class);
            htmlFormEntryService.getHandlers().remove(IntelehealthConstants.HTMLFORMENTRY_ENCOUNTER_DIAGNOSES_TAG_NAME);
            htmlFormEntryService.getHandlers().remove(IntelehealthConstants.HTMLFORMENTRY_ENCOUNTER_DISPOSITION_TAG_NAME);
        } catch (Exception ex) {
            // pass
        }
