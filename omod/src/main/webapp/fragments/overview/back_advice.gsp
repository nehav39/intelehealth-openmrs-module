...
<div id="advice" class="long-info-section"  ng-app="adviceSummary" ng-controller="AdviceSummaryController">
	{{ data }}<br>
	{{ data2 }}<br>
	{{ data3 }}<br>
	{{ myHeader }}
...
<script>
var app = angular.module('adviceSummary', ['ngAnimate', 'ngSanitize', 'ui.bootstrap']);
...
app.controller('AdviceSummaryController', function(\$scope, \$http, \$filter, \$timeout, AdviceSummaryFactory1, AdviceSummaryFactory2) {
  \$scope.alerts = [];
  var _selected;
  var patient = "${ patient.uuid }";
  var date2 = new Date();
...
<script>
    angular.bootstrap("#advice", ['adviceSummary']);
</script>  
