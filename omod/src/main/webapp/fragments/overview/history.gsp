...
<div id="history" class="long-info-section"  ng-app="historySummary" ng-controller="HistorySummaryController">
	<br/>
	...
	<table>
		<tr ng-repeat="item in objects | orderBy:'-encounterDatetime'">
			<td width="100px" style="border: none">{{item.display | dateFormat}}</td>
	                <td style="border:none" ng-repeat="ob in item.obs | filter : 'MEDICAL HISTORY' | orderBy:'-display'">
...
app.controller('HistorySummaryController', function(\$scope, \$http) {
...
<script>
    angular.bootstrap("#history", ['historySummary']);
</script>  
